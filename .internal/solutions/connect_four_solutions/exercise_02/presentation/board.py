...
import colorama
from connect_four_solutions.exercise_02.domain import board, enums
...
def _colorize_token(token: enums.Token) -> str:
...
def generate_board_string(board_state: board.BoardState) -> str:
    ...
    board_string.append(
        f"{separator} " + f" {separator} ".join(ascii_board[-1]) + f" {separator}"
    )
